...
describe('Read Tasks from Excel FIle', () => {
    before(() => {
        //Read data from excel file , covert and write it to a json
        cy.task('readXlsx', { file: 'D:/cypressAutomation/cypress/fixtures/excelData.xlsx', sheet: "Sheet1" }).then((rows) => {
            rowsLength = rows.length;
            cy.writeFile("D:/cypressAutomation/cypress/fixtures/xlsxData.json", { rows })
        })
    })


    beforeEach(() => {
        //visit url every before each test case with a waittime of 2secs
        cy.wait(2000)
        todopage.visit()
    })
